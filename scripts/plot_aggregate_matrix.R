...
# plot_aggregate_matrix: produces a plot of mean ATAC-seq signal
...
p <- ggplot(df, aes(x=Position, y=MeanCutPointCount, colour=Strand))
p <- p + geom_line(alpha=0.75) + facet_grid(Bin ~ Bound, scales="free") + ggtitle(title)
p <- p + xlab('Relative Position') + ylab('ATAC-seq mean signal per region')
